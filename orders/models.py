...
class Order(TimeStampedModel):
    ...
    ORDER_TYPE_SET = set(["cafe", "package", "cake"])
    ...
    status = models.CharField(
        max_length=50, choices=STATUS_TYPE, blank=True, default="not_confirmed"
    )
...
class PackageOrder(TimeStampedModel):
    order = models.OneToOneField(
        "Order", on_delete=models.CASCADE, related_name="packageorders"
    )
...
class OrderedProduct(TimeStampedModel):
    package_order = models.ForeignKey(
        "PackageOrder", on_delete=models.CASCADE, related_name="orderedproducts"
    )
...
class CafeOrder(TimeStampedModel):
    order = models.OneToOneField(
        "Order", on_delete=models.CASCADE, related_name="cafeorders"
    )
...
class CakeOrder(TimeStampedModel):
    order = models.OneToOneField(
        "Order", on_delete=models.CASCADE, related_name="cakeorders"
    )
