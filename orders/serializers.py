...
from .models import Order, PackageOrder, OrderedProduct, CafeOrder, CakeOrder, Review
...
class PackageOrderSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        instance.is_packaging = validated_data.get(
            "is_packaging", instance.is_packaging
        )
        ...
        if orderedproducts:
            ...
            existing_products_id_set = set(
                OrderedProduct.objects.filter(package_order=instance).values_list(
            ...
            OrderedProduct.objects.filter(
                package_order=instance, product_id__in=delete_id_set
            ).delete()

            add_id_set = new_products_id_set - existing_products_id_set
            for id in add_id_set:
                OrderedProduct.objects.create(
                    package_order=instance, product_id=id, buying=True
                )
...
class CakeOrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CakeOrder
        fields = ["id", "product_id", "product_name", "want_pick_up_date", "count"]
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance, *args, **kwargs):
        """Delete not used order field and check is_staff"""

        res = super().to_representation(instance)
        res = self.delete_not_used_order_field(res)

        # for detail check
        if "is_staff" in self.context:
            res["is_staff"] = self.context.get("is_staff")

        return res

    def delete_not_used_order_field(self, res):
        """Delete not used order field in result"""
        type_set = Order.ORDER_TYPE_SET
