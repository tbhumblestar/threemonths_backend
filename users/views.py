...
class KaKaoLoginView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            profile_image_url = (
                response.get("kakao_account").get("profile").get("thumbnail_image_url")
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if User.objects.filter(email=email):
            ...
            if user.login_type != login_type:
                return Response(
                    {"message": f"This email is registered as {user.login_type}."},
            ...
            if not user.check_password(password):
                return Response(
                    {"message": "There are no users matching the information"},
                    status=status.HTTP_401_UNAUTHORIZED,
                )
...
class CheckSMSAuthView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if SMSAuth.objects.filter(contact_num=contact_num, sms_check_num=sms_check_num):
            SMSAuth.objects.filter(
                contact_num=contact_num, sms_check_num=sms_check_num
            ).delete()
    ...
    @extend_schema(
        ...
        examples=[
            OpenApiExample(
                ...
                description="번호(contact_num)는 문자형",
                request_only=True,
                value={"contact_num": "010-0000-0000", "email": "test@test.com"},
            ),
            OpenApiExample(
                name="요청성공(200)",
    ...
    @extend_schema(
        ...
        examples=[
            OpenApiExample(
                ...
                description="번호(contact_num)는 문자형",
                request_only=True,
                value={"user_id": 75, "new_pw": "!abcd2345A!"},
            ),
        ],
        request=inline_serializer(
            "SetNewPW",
